...
1. ImyvmAnnounceLite 是 Fabric Minecraft 服务端 mod，使用 Kotlin 编写，入口为 `com.imyvm.ial.ImyvmAnnounceLite`。
2. 配置类为 `com.imyvm.ial.ImyvmAnnouceLiteConfig`，基于 Hoki `HokiConfig`，配置文件为 `config/imyvm-annouce.conf`。
3. 命令在 `com.imyvm.ial.commands.CommandRegistry.kt` 的 `register()` 中注册。
4. 玩家可见消息集中在 `com.imyvm.ial.util.MsgCommandResponds`。
